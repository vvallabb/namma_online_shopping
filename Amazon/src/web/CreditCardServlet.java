...
public class CreditCardServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		int balance = creditCard.getBalance(cardHolderName, cardNumber, cvv,
				cardPin, bankName);
		...
		if (balance == -1) {
			...
			publisher.putValues(
					(ArrayList) session.getAttribute("ShoppingCartarr"),
					(String) session.getAttribute("uname"),
					(String) session.getAttribute("shippingaddress"));
			ShoppingCartDAO shoppingCartDAO = new ShoppingCartDAO();
			shoppingCartDAO.deleteShoppingCart((String) session
					.getAttribute("uname"));
			RequestDispatcher rd = request
