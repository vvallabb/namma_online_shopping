...
public class ShippingAddressServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (addressvalue.equals("1")) {
			ArrayList userAddrDetails = (ArrayList) session
					.getAttribute("ShippingAddressarr");
			...
			while (userAddrIterator.hasNext()) {
				...
				session.setAttribute(
						"FullName",
						ud.getUserDetailsFirstName()
								+ ud.getUserDetailsMiddleName()
								+ ud.getUserDetailsSurName());
				session.setAttribute("AddressLine1",
						ud.getUserDetailsAddressLine1());
				session.setAttribute("AddressLine2",
				...
				address = ud.getUserDetailsAddressLine1() + ","
						+ ud.getUserDetailsAddressLine2() + ","
			...
			address = addrLine1 + "," + addrLine2 + "," + city + "," + state
					+ "," + zip + "," + country;
		}
		 ...
		 * publisher.putValues((ArrayList
		 * )session.getAttribute("ShoppingCartarr"),(String)
		 * session.getAttribute("uname"),address); ShoppingCartDAO
		...
		RequestDispatcher rd = request
				.getRequestDispatcher("credit_details.jsp");
