...
public class ItemDAO {
	...
	public ArrayList getBooks(ArrayList itemArr, Connection c) {
		...
		while (itemIterator.hasNext()) {
			sBooks = "select * from books b,item i where b.book_id = i.item_id and i.item_id="
					+ itemIterator.next();
			try {
				...
				while (rs1.next()) {
					...
					book.setDigital_list_price(rs1
							.getFloat("digital_list_price"));
	...
	public int getBookCount(ArrayList itemArr, Connection c) {
		...
		int count = 0;
		while (itemIterator.hasNext()) {
			sBooks = "select 1 from books b,item i where b.book_id = i.item_id and i.item_id="
					+ itemIterator.next();
			try {
				PreparedStatement ps = c.prepareStatement(sBooks);
				ResultSet rs1 = ps.executeQuery();
