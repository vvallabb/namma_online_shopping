...
public class SubcategoryDAO {
		...
		try {
			...
			while (rs1.next()) {
				if (!hcat.containsKey(rs1.getString("Category_name"))) {
					...
					ArrayList list = (ArrayList) hcat.get(rs1
	...
	public ArrayList getAllItemsForSubcategory(String subcategoryName,
			Connection c) {

		String sCategories = "select subcategory_id from subcategory where subcategory_name='"
				+ subcategoryName + "'";
		...
		try {
			...
			String sItems = "select item_id from item where subcategory_id="
					+ subcategoryId;
	...
	public String getCategory(String subcategoryName, Connection c) {
		...
		String sCategories = "select c.Category_name from subcategory s,category c where s.subcategory_name='"
				+ subcategoryName + "' and s.category_id=c.Category_id";
		try {
			PreparedStatement ps = c.prepareStatement(sCategories);
			ResultSet rs1 = ps.executeQuery();
			int categoryId = 0;
